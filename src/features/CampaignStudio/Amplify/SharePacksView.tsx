...
export const SharePacksView: React.FC<SharePacksViewProps> = ({ packs, onOpen, onSend }) => {
  ...
  const primaryAction = (pack: SharePack) => {
    ...
    if (pack.status === "ready" || pack.status === "draft") return "Send";
    return "View";
  };
  ...
  return (
    <div className="amp-packs">
      ...
      <div className="amp-pack-grid">
        {filtered.map((pack) => (
          <article key={pack.id} className="amp-pack-card">
            ...
            <div className="amp-pack-card__actions">
              <button
                ...
                onClick={() => {
                  if (pack.status === "ready" || pack.status === "draft") onSend([pack.id]);
                  else onOpen(pack);
                }}
              >
                {primaryAction(pack)}
