...
/** Skip green cultural anchors when AI draft cards already represent that event. */
const uncoveredEventsForDay = (day: Date, year: number, cards: AdvisorCard[]) => {
  const coveredEventIds = new Set(
    cards.filter((card) => card.source === "cultural" && card.eventId).map((card) => card.eventId as string),
  );
  return eventsForDay(day, year).filter((event) => !coveredEventIds.has(event.id));
};
...
const DayView: React.FC<ViewProps> = ({
  ...
  const events = uncoveredEventsForDay(focusDate, year, cards);
  ...
  return (
    <div className="cb-view cb-view--day cb-fade-in">
      <CalendarHeader
        ...
        subtitle="AI recommendations for the selected day."
        horizon={horizon}
    ...
    <div className="cb-view cb-view--week cb-fade-in">
      ...
      <div className="cb-week-grid">
        {days.map((day) => {
          ...
          const cards = cardsByDate.get(iso) || [];
          const events = uncoveredEventsForDay(day, year, cards);
          return (
  ...
  return (
    <div className="cb-view cb-view--month cb-fade-in">
      <CalendarHeader
        ...
        subtitle="AI drafts on the cultural calendar. Click any card to review."
        horizon={horizon}
        onHorizonChange={onHorizonChange}
        onShiftFocus={onShiftFocus}
      />
            ...
            <section key={monthIndex} className="cb-quarter-month">
              ...
              {monthEvents.length === 0 ? (
                ...
                monthEvents.map(({ event, date }) => {
                  ...
                  const relatedCards = (cardsByDate.get(iso) || []).filter((card) => card.eventId === event.id);
                  return (
